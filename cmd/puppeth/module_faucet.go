...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	...
	(cd $GOPATH/src/github.com/ethereum && git clone --depth=1 https://github.com/ethersocial/go-ethersocial) && \
  go build -v github.com/ethersocial/go-ethersocial/cmd/faucet                                              && \
